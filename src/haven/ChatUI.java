...
public class ChatUI extends Widget {
	...
	public ChatUI(int w, int h) {
		super(new Coord(w, h));
		chansel = add(new Selector(new Coord(selw - UI.scale(10), sz.y - marg.y)), marg);
		setfocusctl(true);
		setcanfocus(true);
		if (h < 1)
			hide();
		...
		public class NamedMessage extends Message {
			...
			public Text text() {
				...
				if ((r == null) || !nm.equals(cn)) {
					r = fnd.render(RichText.Parser.quote(String.format("[%s] %s: %s", NUtils.timestamp(), nm, text)), w, TextAttribute.FOREGROUND, col);
					cn = nm;
				}
				return (r);
			}
	...
	public <T extends Widget> T add(T w) {
		if (w instanceof Channel) {
			...
			chan.resize(sz.x - marg.x - chan.c.x, sz.y - chan.c.y - UI.scale(50));
			super.add(w);
			select(chan, false);
			chansel.add(chan);
			return (w);
		...
		public void draw(GOut g) {
			...
			synchronized (chls) {
				while (i < chls.size()) {
					...
					if (ch.chan == sel)
						g.image(chanseld, Coord.of(x - chanseld.sz().x / 2, 5));
					...
					int mx = x + UI.scale(8);
					if (icon == null) {
						g.aimage(name, Coord.of(x - name.sz().x / 2, sz.y / 2), 0, 0.5);
					} else {

//                        g.aimage(icon, Coord.of(x-name.sz().x/2, sz.y / 2), 0.0, 0.5); x += icon.sz().x;
						g.aimage(name, Coord.of(x - name.sz().x / 2, sz.y / 2), 0.0, 0.5);
					}
					x += chanseld.sz().x;
					if (x >= sz.x)
						break;
...
//	this.c = base.add(0, -this.sz.y);
		chansel.resize(new Coord(sz.x - UI.scale(18), UI.scale(25)));
		if (sel != null)
	...
	public void move(Coord base) {
		this.c = (this.base = Coord.z).add(0, 0);
	}
